...
```{r glimpse}
# Graphique G1 : Distribution des prix des maisons
...
# Affichage du graphique G1
print(g1)


# Gaphique G2 : Relation Prix ~ Surface avec colorisation par 'view'
g2 <- ggplot(house, aes(x = sqft_living, y = price, color = factor(view))) +
  geom_jitter(alpha = 0.5) +
  geom_smooth(method = "lm", se = FALSE, color = "red") +
  theme_minimal() +
  labs(
    title = "G2 : Relation entre le prix et la superficie habitable",
    x = "Surface habitable (sqft)",
    y = "Prix (USD)",
    color = "Vue (0-4)"
  ) +
  theme(
...
# Affichage du graphique G2
print(g2)

# Graphique D1 : Nombre de maisons par catégorie (nombre de chambres)
library(dplyr)
...
house_summary <- house %>%
  ...
  filter(percentage > 1)  # Exclusion des catégories avec moins de 1%

# Palette de couleurs personnalisée
couleurs_personnalisees <- c(
...
# Graphique D1 avec couleurs personnalisées
d1 <- ggplot(house_summary, aes(x = factor(bedrooms), y = count, fill = factor(bedrooms))) +
  geom_bar(stat = "identity", color = "black", width = 0.7) +
  geom_text(aes(label = paste0(count, " (", percentage, "%)")), 
            vjust = -0.5, size = 3.5, color = "black") +  # Pourcentages et noms
  scale_fill_manual(values = couleurs_personnalisees) +
  theme_minimal() +
  labs(
    title = "D1 : Nombre de maisons par catégorie (nombre de chambres)",
  ...
  theme(
    ...
    legend.position = "none"  # Masquage de la légende pour plus de clarté
  )

# Affichage du graphique D1
print(d1)

# Création du Graphique D2 : Nombre de Maisons par Grade (Qualité de la Construction)

# Calcul des pourcentages pour chaque grade
house_grade_summary <- house %>%
  group_by(grade) %>%
  summarize(count = n()) %>%
  mutate(percentage = round((count / sum(count)) * 100, 1))

# Graphique D2 avec palette automatique
d2 <- ggplot(house_grade_summary, aes(x = factor(grade), y = count, fill = factor(grade))) +
  geom_bar(stat = "identity", color = "black", width = 0.7) +
  geom_text(aes(label = paste0(count, " (", percentage, "%)")), 
            vjust = -0.5, size = 3.5, color = "black") +  
  scale_fill_viridis_d(option = "D") +  # Palette automatique
  theme_minimal() +
  labs(
    title = "D2 : Nombre de maisons par grade (Qualité de la construction et du design)",
