...
#library(leaflet)
#leaflet(house) %>% addTiles() %>% addCircleMarkers(~long, ~lat,radius = 5,opacity = 0.001, popup = ~as.character(price))


milieu.lat = (max(house$lat)+min(house$lat))/2
milieu.long = (max(house$long)+min(house$long))/2
max.long = max(house$long)
min.long = min(house$long)
max.lat = max(house$lat)
min.lat = min(house$lat)



sud_ouest <-  house %>%
  filter((min.lat<house$lat & house$long<milieu.lat) & (min.long<house$long & house$long<milieu.long))

sud_est <-  house %>%
  filter((min.lat<house$lat & house$long<milieu.lat) & (milieu.long<house$long & house$long<max.long))

nord_est <- house %>%
  filter((milieu.lat<house$lat & house$lat<max.lat) & (milieu.long<house$long & house$long<max.long))

nord_ouest <- house %>%
  filter(((milieu.lat<house$lat | house$lat<max.lat) & (min.long<house$long | house$long<milieu.long)))
house <- house %>%
  mutate(region = case_when(
    lat >=  milieu.lat & long >=  milieu.long ~ "Nord Est",
    lat >=  milieu.lat & long <=  milieu.long ~ "Nord Ouest",
    lat <=  milieu.lat & long >=  milieu.long ~ "Sud Est",
    lat <=  milieu.lat & long <=  milieu.long ~ "Sud Ouest",
    TRUE ~ "Autre"
  ))
...
#```{r cluster découpage zones géographiques}
#
#set.seed(2025) 
#kmeans_result <- kmeans(house[, c("long", "lat")], centers = 10)
#
## Ajoutons les labels de zones au jeu de données
#house_clean$zone <- as.factor(kmeans_result$cluster)
#
## Visualisons les zones
#p <- ggplot(house, aes(x = long, y = lat, color = zone), text = paste("Zone:", zone)) +
#  geom_point(alpha = 0.6) +
#  expand_limits(x=seq(-123,-121, by=0.25))+
#  #expand_limits(y=47, 0.2)+
#  labs(
#    title = "Répartition des maisons par zone géographique (10 clusters)",
#    x = "Longitude",
#    y = "Latitude",
#    color = "Zone"
#  ) +
#  theme_minimal()
#
## Rendons le graphique interactif avec Plotly
