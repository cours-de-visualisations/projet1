...
#d3 <- ggplot(house, aes(x = "", fill = factor(waterfront))) +
d3 <-  ggplot(house_waterfront, aes(x = factor(waterfront), y = count, fill = factor(waterfront))) +  
  geom_bar(width = 0.7, stat = "identity", color = "black" ) +  
  coord_polar(theta = "y") + 
  scale_fill_viridis_d(option = "D") + 
  labs(
    title = "D3: Répartition des maisons avec ou sans vue sur l'eau",
    fill = "Vue sur l'eau (0 = Non, 1 = Oui)"
  ) +
  theme_minimal() +
  theme(
...
d4 <- ggplot(house_condition, aes(x = factor(condition), y = count)) +
  ...
  theme_minimal() +
  labs(
    title = "D4 : Nombre de maison selon l'état  general (condition)",
    x = "condition",
    y = "Nombre de maisons",
    fill = "Nombre de chambres"
  ) +
